...
for i in range(n):
    for j in range(n):
        for t in interval(0,l):
            ...
            if (j==(n-1) and j>0 and t>0 and t<l):
                # pos_c.append(Implies(h[i][j][t], Or(And(h[i][j-1][t-1],m[i][j][t]),h[i][j][t-1])))
                # pos_c.append(Implies(h[i][j][t], Or(And(h[i][j-1][t+1],m[i][j][t+1]),h[i][j][t+1])))
                pos_c.append(Implies(h[i][j][t],h[i][j][t-1]))
                pos_c.append(Implies(h[i][j][t],h[i][j][t+1]))
...
for i in range(n):
    for j in range(n):
        for t in interval(0,l):
            ...
            if (j==(n-1) and j>0 and t>0 and t<l):
                # pos_c.append(Implies(r[i][j][t], Or(And(r[i][j-1][t-1],m[i][j][t]),r[i][j][t-1])))
                # pos_c.append(Implies(r[i][j][t], Or(And(r[i][j-1][t+1],m[i][j][t+1]),r[i][j][t+1])))
                pos_c.append(Implies(r[i][j][t],r[i][j][t-1]))
                pos_c.append(Implies(r[i][j][t],r[i][j][t+1]))
...
for i in range(n):
    for j in range(n):
        for t in interval(0,l):
            ...
            if (i==(n-1) and i>0 and t>0 and t<l):
                pos_c.append(Implies(v[i][j][t], Or(And(v[i-1][j][t-1],m[i][j][t]),v[i][j][t-1])))
                pos_c.append(Implies(v[i][j][t], Or(And(v[i-1][j][t+1],m[i][j][t+1]),v[i][j][t+1])))
